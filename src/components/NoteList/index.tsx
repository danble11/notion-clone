...
export function NoteList({ layer = 0, parentId }: NoteListProps) {
  ...
  const[expanded, setExpandede] = useState<Map<number, boolean>>(new Map());
  ...
  const createChild = async (e:React.MouseEvent, parentId: number) =>{
    ...
    setExpandede((prev) => prev.set(parentId, true));
      ...
      map((note) => {
        return (
          <div key={note.id}>
            ...
            <NoteList layer={layer + 1} parentId={note.id}></NoteList>
            )}
          </div>
        );
      })}
