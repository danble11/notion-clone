 ...
 */
function sinkListItem(itemType, groupType) {
    return function ({ state, dispatch }) {
        ...
        if (dispatch) {
            ...
            const slice = new Slice(Fragment.from(itemType.create(null, Fragment.from(groupType.create(null, inner))) // change necessary to create "groupType" instead of parent.type
            ), nestedBefore ? 3 : 1, 0);
...
export function nestBlock(editor) {
    return editor._tiptapEditor.commands.command(sinkListItem(editor._tiptapEditor.schema.nodes["blockContainer"], editor._tiptapEditor.schema.nodes["blockGroup"]));
}
...
export function canNestBlock(editor) {
    const { bnBlock: blockContainer } = getBlockInfoFromSelection(editor._tiptapEditor.state);
    return (editor._tiptapEditor.state.doc.resolve(blockContainer.beforePos)
        .nodeBefore !== null);
}
